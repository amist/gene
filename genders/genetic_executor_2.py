...
class Population:
    ...
    def _expand_population(self):
        for _ in range(self.size * (self.expansion_factor - 1)):
            ...
            children = parent2.get_children(parent1)
            for child in children:
                self.population_a.append(child)
...
class Individual:
    def get_random_individual(self):
        raise NotImplementedError('You have to implement a get_random_individual function')
...
class GeneticExecutor:
    ...
    def get_solution(self):
        ...
        for i in range(self.max_generations_number):
            ...
            print('  Current maximum fitness value in population_a = {} with {} | {}'.format(population.population_a[0].get_fitness_value(), population.population_a[0].data_chromosome, population.population_a[0].confidence_chromosome))
            print('  Current maximum fitness value in population_b = {} with {} | {}'.format(population.population_b[0].get_fitness_value(), population.population_b[0].data_chromosome, population.population_b[0].confidence_chromosome))
